...
pub(crate) fn get_lat_lon_and_values(byte_string: &[u8], message_index: (usize, usize)) -> Result<(Vec<f32>, Vec<f32>, Vec<f32>), GribViewerError>
{
    // Parse the GRIB2 message.
    let grib2 = grib::from_bytes(byte_string)?;
...
pub(crate) fn find_grib_index(bytes: &[u8], discipline: u8, category: u8, parameter: u8, time: i64) -> Result<(usize, usize), GribViewerError> {
